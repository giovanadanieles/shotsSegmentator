...
void writeNewShots(const pair<string, string> shot[N], ofstream &outputFile, int count){
	...
	if(count == 1){
		...
		range[0] = floor((sec[1] - fir[1]) * 33 / 100);
		...
		range[0] = floor((sec[1] - fir[1]) * 33 / 100);
		range[1] = floor((sec[2] - fir[2]) * 66 / 100);

		if(range[0] > 25 && range[1] > 25){
			outputFile << fir[0] << "\t" << sec[0] + range[0] << endl;

			outputFile << sec[0] + range[0] + 1 << "\t" <<
						  sec[0] + range[0] + 1 + range[1] << endl;

			outputFile << sec[0] + range[0] + 2 + range[1] << sec[2] << endl;
		}
		else{
			outputFile << fir[0] << "\t" << sec[0] << endl;
			outputFile << fir[1] << "\t" << sec[1] << endl;
		...
		if(range[0] > 25 && range[1] > 25){
			...
			outputFile << sec[0] + range[0] + 2 + range[1] << "\t" << sec[3] << endl;
		}
		else{
			outputFile << fir[0] << "\t" << sec[0] << endl;
			outputFile << fir[1] << "\t" << sec[1] << endl;
			outputFile << fir[2] << "\t" << sec[2] << endl;
			outputFile << fir[3] << "\t" << sec[3] << endl;
		}
	}
}
